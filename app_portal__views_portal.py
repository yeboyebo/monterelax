...
class monterelax(interna):

    def monterelax_login(self, request, error=None):
        """ Peticion defecto"""
        if not error:
            error = ''
        return render(request, 'portal/login.html', {'error': error})

    def monterelax_auth_login(self, request):
    ...
    def monterelax_account_request(self, request):
        return HttpResponseRedirect("/")
    ...
    def auth_login(self, request):
        return self.ctx.monterelax_auth_login(request)

    def account_request(self, request):
        return self.ctx.monterelax_account_request(request)

    def login(self, request, error=None):
        return self.ctx.monterelax_login(request, error)
